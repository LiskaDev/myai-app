...
- **🔗 跨场景记忆同步** — 群聊重要事件自动写入角色永久记忆，单聊也能记得

### 📔 日记系统
- **角色日记** — AI 以角色视角撰写日记，记录当天情感与事件
- **日记链** — 新日记自动延续旧日记中的目标与情绪，避免遗忘
- **一键开启新一天** — 🌙 菜单开启下一天，角色自动以新一天的状态回应
- **历史日记浏览** — 按角色/群聊过滤，沉浸式翻页阅读

### 👤 全局用户画像
- **自动特征提取** — AI 后台分析聊天记录，提取用户偏好、性格、事实等
...
- **Tab 分页设置** — 角色/记忆/画像/通用/数据 五栏切换，告别拥挤
...
│   │   ├── usePromptBuilder.js    # System Prompt 构建 + 画像注入
...
│   │   └── settings/              # 设置子组件
│   │       ├── MemoryManager.vue  # 记忆管理 + 来源标识
│   │       └── UserPersonaSettings.vue # 用户画像标签管理
│   └── utils/
│       ├── storage.js             # localStorage 封装 + 画像存储
│       ├── textParser.js          # RP 文本格式化
│       ├── markdown.js            # Markdown 渲染 + DOMPurify
│       ├── validation.js          # 输入验证 + 安全
│       └── summary.js             # 摘要工具函数
├── tests/                          # 单元测试 (122 cases)
│   ├── textParser.test.js
│   ├── useChat.test.js
...
#### ✨ 新功能
- **📔 日记系统** — AI 以角色视角撰写日记，支持日记链延续目标与情绪
- **☀️ 开启新一天** — 🌙 菜单一键进入下一天，AI 自动以新一天状态回应
- **👤 全局用户画像** — 后台自动分析用户特征（偏好/性格/事实/风格/边界），注入 Prompt
- **🔗 跨场景记忆同步** — 群聊重要事件（表白、承诺、争吵）自动写入角色永久记忆
- **🗜️ 智能记忆压缩** — 记忆超过 20 条时自动压缩最旧 10 条为精炼摘要，防止 Token 爆炸
- **记忆来源标识** — 🔗 群聊同步 / 📝 手动创建 / 🎭 消息钉选 / 📦 历史摘要

#### 🐛 Bug 修复
- **日记过滤** — 每个对话只显示自己的日记，群聊只显示群内角色的日记
- **HTML 多重转义** — 群聊消息 `<span>` 标签被反复转义为 `&amp;lt;span` 的问题
- **新一天角色修正** — 群聊中使用 `director` 角色而非 `user`
