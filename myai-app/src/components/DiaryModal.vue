...
<template>
    ...
    <Transition name="card-panel">
        <div v-if="show && viewMode === 'card'" class="card-overlay" @click.self="handleClose">
            <div class="card-panel">
                <div class="panel-handle"></div>
                <button class="panel-close-btn" @click="handleClose">✕</button>

                <div class="panel-title">📖 角色日记</div>
    ...
    <!-- ── paper 模式：居中信纸弹窗（原样不动） ── -->
    <Transition name="diary-fade">
        <div v-if="show && viewMode === 'paper'" class="diary-overlay" @click.self="handleClose">
            <Transition name="diary-slide" appear>
                <div class="diary-container" v-if="show && viewMode === 'paper'">
                    <div class="diary-paper">
                        ...
                        <div v-else-if="currentDiary" class="diary-content-area">
                            ...
                            <div class="diary-signature">—— {{ currentDiary.roleName }}</div>
                        </div>
...
/* 面板滑入动画 */
.card-panel-enter-active { transition: opacity 0.25s; }
.card-panel-leave-active { transition: opacity 0.25s; }
.card-panel-enter-from,
.card-panel-leave-to { opacity: 0; }
.card-panel-enter-active .card-panel,
.card-panel-leave-active .card-panel { transition: transform 0.32s cubic-bezier(0.25, 1, 0.4, 1); }
.card-panel-enter-from .card-panel,
.card-panel-leave-to .card-panel { transform: translateY(100%); }
...
.diary-loading-text {
    font-family: 'Ma Shan Zheng', cursive, serif;
    color: #8b6e52; font-size: 1.1rem; margin-top: 12px;
...
.diary-author-name {
    font-family: 'Ma Shan Zheng', cursive, serif;
    font-size: 1.15rem; color: #5a3e2b; font-weight: 600;
}
.diary-date { font-size: 0.75rem; color: #a08060; margin-top: 2px; }
.diary-absence-badge {
...
.diary-page-btn {
    ...
    color: #5a3e2b; font-size: 0.8rem;
    cursor: pointer; transition: all 0.2s;
}
.diary-page-btn:hover:not(:disabled) { background: rgba(180, 150, 100, 0.2); }
.diary-page-btn:disabled { opacity: 0.3; cursor: default; }
.diary-page-info { font-size: 0.8rem; color: #a08060; }

/* paper 模式动画 */
.diary-fade-enter-active,
.diary-fade-leave-active { transition: opacity 0.35s ease; }
...
@keyframes diarySlideIn {
    from { opacity: 0; transform: translateY(30px) scale(0.92) rotateX(8deg); }
