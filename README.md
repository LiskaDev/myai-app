...
> **沉浸式 AI 角色扮演对话平台** — 与 AI 角色进行视觉小说体验，深度角色定制和智能记忆系统
...
### 🆕 DeepSeek 多模态图片消息
- **新增视觉模型预设** — API 配置的主模型中新增实验模型 `deepseek-v4-flash-vision-exp`
- **图片消息** — 使用该模型时，可在单聊输入区选择并发送图片，也支持只发送图片；已发送图片可点击放大预览
- **移动端兼容** — 现代 iOS / Android 浏览器会调用系统相册或文件选择器，图片在浏览器本地处理后再发送
- **发送前处理** — 普通图片自动缩放至最长边 1280px 并转换为 WebP，以降低流量和上下文开销；GIF 保留原格式
- **安全限制** — 仅接受 JPEG、PNG、WebP、GIF；每条消息最多 4 张；普通图片最大 12MB，GIF 最大 4MB；无法解码或伪装成图片的文件会被拒绝

> [!IMPORTANT]
> 图片输入仅对 `deepseek-v4-flash-vision-exp` 开放。切换到其他模型后，历史图片不会再次发送给模型，仅保留文字上下文，以避免接口报错。
>
> 当前聊天使用 Chat Completions API，**不自带联网搜索**。视觉模型可以根据图片内容和已有知识进行描述或识别，但不能实时联网核实作品、人物或新闻；识别冷门、模糊或较新的内容时可能出错。
>
> 该模型属于实验模型，名称、能力、价格及可用性以后可能由服务商调整。图片会随聊天记录保存在当前浏览器的本地数据中，并作为请求内容发送到用户配置的 DeepSeek API，请勿上传敏感图片。对于文件体积较小但像素尺寸异常巨大的图片，低内存手机仍可能在解码时出现卡顿，建议使用常规相机照片或截图。
...
- **酒馆角色卡兼容** — 支持直接导入 SillyTavern 格式的第三方角色卡（`.png` 图像内嵌数据或 `.json` 格式）
...
### 💾 数据管理
- **混合式本地存储引擎** — 最易膨胀的**核心数据片段**（角色设定与海量对话历史、全局设置）进驻异步 `IndexedDB`，单机即可轻松储存数百 MB 文本记录，彻底告别 5MB 瓶颈。
- **辅助数据独立缓存** — 世界书、角色日记、小说书库引擎、用户人设与卡片库等模块继续沿用 `localStorage` 缓存。在控制变更风险、保障主聊天数据安全的同时，实现了完全无需后端的纯客户端体验。
